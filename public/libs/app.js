...
function getSpaces(callback) {
	...
	.done(function(response) {
		if (response.responseCode === 0) {
			...
			$('#message').html("You must verify your identity via a Webex message.");
			paintEmailInput();
		}
	})
	.fail(function() {
...
function paintSpacesList(spaces) {
	...
	spaces.forEach(function(space){
		...
		var spaceHtml = $("#spaceTemplate").html()
			...
			.replace("%TEXT%", space.title.replace(/(<([^>]+)>)/g, "").truncString(55))
...
function checkInstalled(state) {
	...
	if (shortId != '')
		...
		if (state == true)
			$('#message').html("Follow the link in the Webex message you just got to verify your identity.");
		else {
			$('#message').html("Follow the link in the Webex message you just got to verify your identity.");
      	...
      	var html = "<button class='btn btn-lg btn-block btn-success' onClick=\"window.location = '"+teamsUrl+"'\">Get Webex</button>"
			$('#input').html(html);
		}
	}
}
	...
	.done(function(data){
		...
		switch (data.responseCode) {
			case 0:
				...
				break;
			case 12:
				$('#message').html("Email is not Webex enabled. Contact your IT administrator.");
				paintEmailInput();
				break;
			case 14:
				$('#message').html("Bot emails cannot be used");
				paintEmailInput();
				break;	
			default:
...
function searchSpaces(searchText, internalOnlyState, spaces) {
	// EXPLORE PLUGIN OPTIONS: https://www.fusejs.io/api/options.html
	...
	// Refresh the space list before filtering
	document.querySelectorAll('a.internal-true').forEach(function(el) {
		$(el).toggle(false);
  });
	...
	if(searchText.length > 0) {
		result = fuse.search("'" + searchText + " | =" + searchText + " | ^" + searchText[0]);
	}
	result.forEach(function(obj){
		var elem = $('[href="./#' + (obj.shortId || obj.item.shortId) + '"]');

		var show;
		if(elem !== undefined)
			show = true;

		if (internalOnlyState && elem.hasClass('internal-false'))
			show = false;
...
function paintSearchInput(spaces) {
	$('#input').html(
	...
	$('#internalFilter').on('click', function() {
		...
		searchSpaces($('#searchInput').val(), internalOnly, spaces);
	});
	$('#searchInput').on('keyup', function(){
		searchSpaces($(this).val(), internalOnly, spaces);
	});
	$('#searchInput').focus();
	if (
...
*/
	...
	if (teamsUrl !== null) {
		var successText;
		if (installed == 'true') successText = 'Open Webex';
		else successText = 'Get Webex';
	...
	switch (data.responseCode) {
		case 0:
			setEmail();
			$('#message').html('Added to Webex space');
		...
		case 5:
			setEmail();
			$('#message').html("Already in Webex space");//+'<br/><span>'+spaceTitle+'</span><br/><span>'+message+'</span>');
			$('#input').html(successHtml);
			break;
		case 6:
			setEmail();
			var message = "Couldn't add to Webex space<br/>Admins have been notified";
			$('#message').html(message);
			$('#input').html('');
			break;
		case 7:
			setEmail();
			var message = "Added to Webex space<br/>Check your email for instructions";
			$('#message').html(message);
			$('#input').html('');
			break;
		case 8:
			setEmail();
			var message = "Couldn't add to Webex space";
			if (supportEmail != '')
		...
		case 12:
			eraseCookie(emailCookie);
			$('#emailContainer').hide();
			var message = "Email is not Webex enabled. Contact your IT administrator.";
			$('#message').html(message);
			$('#input').html('');
			paintEmailInput();
			break;
		case 13:
			...
			setLogo(botAvatar, botName+' ('+botEmail+')', true);
			setEmail();
			var message = "Not permitted to Webex space";
			$('#message').html(message);
		...
		default:
			setEmail();
			var message = "Failed to add to Webex space<br/>Admins have been notified";
			$('#message').html(message);
			$('#input').html('');
			break;
	}
...
function joinSpace(shortId) {
	//var message = "Adding to Webex space";
	$('#message').html("<i class='fa fa-sync fa-spin'></i>");
	$.ajax({
		url : './api/shortid/'+shortId,
		type: "POST",
...
function setup() {
	document.title = botName+' | Join Webex Spaces';
  documentationPopup();
	if (supportEmail != '' || supportUrl != '') {
		$('#support').show();
		var supportSubject = 'Question about joining Webex Space via '+botName+' ('+botEmail+')';
